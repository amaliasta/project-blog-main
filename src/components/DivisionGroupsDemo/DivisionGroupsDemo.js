            ...
            <Card as="section" className={styles.wrapper}>
                ...
                <div className={styles.demoWrapper}>
                    <div
                        ...
                        style={gridStructure}>
                        {range(numOfGroups).map((groupIndex) => { // 0 1 2 3 
                            const firstGroupIndex = numOfItemsPerGroup * groupIndex;
                            const lastGroupIndex = numOfItemsPerGroup + firstGroupIndex;
                            return (
                                <div key={groupIndex} className={styles.group}>
                                    {range(firstGroupIndex, lastGroupIndex).map((index) => {
                ...
                {includeRemainderArea && (
                    <div className={styles.remainderArea}>
                        ...
                        {range(numOfItems, numOfItems + remainder, 1, true).map(
                            (index) => {
                                const itemId = `${id}-${index}`;
                                return (
                                    <motion.div
